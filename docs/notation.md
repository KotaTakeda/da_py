...
| Sample covariance | $P_{n}=(m-1)^{-1}A_{n}^{\top}A_{n}$ | $\mathrm{Cov}_{m}(\boldsymbol{V})=(m-1)^{-1}d\boldsymbol{V}d\boldsymbol{V}^{*}$ | ${\boldsymbol{C}}_{n}^{f}={\boldsymbol{V}}_{n}^{f}({\boldsymbol{V}}_{n}^{f})^{\top}/(m-1)$ | da_py arrays are row-major, so $A_{n}^{\top}A_{n}$ gives the state covariance. Takeda thesis uses $^{*}$ for Hilbert-space adjoints. |
...
| Transform matrix | $T$ | $T_{n}\in\mathbb{R}^{m\times m}$ | ${\boldsymbol{T}}_{n}=(\boldsymbol{I}_{m}+(m-1)^{-1}({\boldsymbol{V}}_{n}^{f})^{\top}\boldsymbol{H}^{\top}\boldsymbol{R}^{-1}\boldsymbol{H}{\boldsymbol{V}}_{n}^{f})^{-1/2}$ | ${\boldsymbol{T}}_{n}$ is a matrix square root chosen symmetric positive definite in the paper. |
...
- Kota Takeda, *Error Analysis of the Ensemble Square Root Filter for Dissipative
  ...
  $\overline{v}=m^{-1}\sum_{k} v^{(k)}$, $\overline{v}\mathbf{1}$,
